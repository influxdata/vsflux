...
export class Instance extends vscode.TreeItem {
    ...
    getTreeItem() : Thenable<vscode.TreeItem> | vscode.TreeItem {
        return {
            ...
            description: '2.x',
            collapsibleState: vscode.TreeItemCollapsibleState.Collapsed,
            iconPath: this.instance.cli
                ? new vscode.ThemeIcon('console', this.instance.isActive ? new vscode.ThemeColor('terminal.ansiBlue') : undefined)
                : this.context.asAbsolutePath(`resources/influx-logo${this.status}.svg`),
            contextValue: this.instance.cli ? 'cliinstance' : 'instance'
        }
    }
